...
"""
NOTE: [AI citation]: I did use chatGPT to update my original (bad) idea of storing
incoming/outgoing trade requests in theuser itself, but chatGPT
recommended that I store this separately
"""
...
class Trades:
    ...
    def _get_incoming_for(self, email: str) -> list[Trade]:
        return [
            ...
            if email == trade.receiver_email
        ]

    def _get_outgoing_for(self, email: str) -> list[Trade]:
        return [
            trade for trade in self._trades.values()
            if email == trade.sender_email
        ]
